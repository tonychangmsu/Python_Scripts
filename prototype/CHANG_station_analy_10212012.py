...
class statoutput(object):
    def __ini__(self, stationcode=None, factor=None, startyear=None, endyear=None, gradient=None, intercept=None, rsq=None, pval=None):
...
def data_plot(data,startyear, endyear, factor):
    ...
    for j in range(numstations):
        for k in range(13):
            for i in range(numpoints):
                if (data[i].stationcode == j and data[i].year>=startyear and data[i].year <= endyear and data[i].month == month):
        ...
        rsq = r_value**2
        x = statoutput(stationcode, factor, startyear, endyear, gradient, intercept, rsq, p_value)
        results.append(x)
    """
