...
queue = cl.CommandQueue(ctx)
'''
a_gpu = cl_array.to_device(ctx, queue, np.random.randn(n).astype(np.float32))
b_gpu = cl_array.to_device(ctx, queue, np.random.randn(n).astype(np.float32))
