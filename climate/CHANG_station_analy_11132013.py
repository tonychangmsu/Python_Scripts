        ...
        for i in range(numpoints):           
            if (data[i].stationcode == j and data[i].year>=startyear and data[i].year <= endyear):                               
                ...
                if (temp != None):                
                    ...
                    year_array.append(data[i].year+(data[i].month/12))
        if (len(factor_array) > 0):
            gradient, intercept, r_value, p_value, std_err = stats.linregress(year_array,factor_array)
            rsq = r_value**2
            x = statoutput(j, factor, year_array[0], year_array[len(year_array)-1], gradient, intercept, rsq, p_value)
            results.append(x)
    """
...
def plot_yearlyavg(data,station, startyear, endyear, factor):
    ...
    if (station == "all"): #if station request is for all stations
        ...
        gradient, intercept, r_value, p_value, std_err = stats.linregress(x, y)
        print("Gradient and intercept %0.2f" %(gradient, intercept))
        print("R-squared %0.2f" %(r_value**2))
...
class statoutput(object):
    def __init__(self, stationcode=None, factor=None, startyear=None, endyear=None, gradient=None, intercept=None, rsq=None, pval=None):
        self.stationcode = stationcode
        self.factor = factor
...
#wdata = '/media/PENDRIVE/PhD materials/Combined_data_monthlyPYTHON_10202012.csv' 
wdata = '/media/PENDRIVE/PhD materials/Combined_data_monthlyPYTHON_10202012.csv' 
