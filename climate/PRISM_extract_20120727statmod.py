...
for i in range(df):
    if (Pdata[i].month == 14): # only take the annual averages for this case
        xbar = xbar + (ones((n,m)) * Pdata[i].year)
        ybar = ybar + Pdata[i].PRval.astype(float)/100
xbar = xbar/df
ybar = ybar/df

for i in range(df-1):
    if (Pdata[i].month == 14):
        Sxx = Sxx +(((ones((n,m)) * Pdata[i].year)-xbar)*((ones((n,m)) * Pdata[i].year)-xbar))
        Sxy = Sxy +((ones((n,m)) * Pdata[i].year)-xbar)*((Pdata[i].PRval.astype(float)/100)-ybar)
