...
def All_monsummary(data,Beginyear, Endyear,min_elev = 0): #specify the minimum elevation of data points
	...
	s_count = np.zeros(((Endyear-Beginyear)+1,12))
	counter = []
	for i in range(len(data)):
		if (data[i].elev>= min_elev): #check if within the elevation range
			for j in range(len(data[i].monsum)):
				if (data[i].monsum[j].year >=Beginyear and data[i].monsum[j].year <= Endyear): #check if within the year range
					...
					counter.append([yr, data[i].monsum[j].year])
					for k in range(len(data[i].monsum[j].month)):
						mon = data[i].monsum[j].month[k]
						swe_m[yr][mon-1] += data[i].monsum[j].SWE[k]
						melt_m[yr][mon-1] += data[i].monsum[j].melt[k]
						zsn_m[yr][mon-1] += data[i].monsum[j].zerosnowdays[k]
						s_count[yr][mon-1] +=1 #add a station count for that data point
	s_count[np.where(s_count==0)] = np.nan #find where there are no station counts
	#find the means for all the summaries
	swe_mean = swe_m/s_count
	melt_mean = melt_m/s_count
	zsn_mean = zsn_m/s_count
	datearray = np.arange(np.datetime64(str(Beginyear)+'-01','M'), np.datetime64(str(Endyear)+'-12','M')+1)
	return(swe_mean, melt_mean,zsn_mean,s_count, datearray, counter)
