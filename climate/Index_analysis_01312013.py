...
Note: issues exist with NAM plot due to -999 values for last months. Can be corrected
using 0 values or not including the data for the 2012 year
"""
import numpy as np
...
TCP = ['AO', 'MEI', 'PDO', 'PNA','NAM_PC' ] #teleconnection pattern list
T = 1 # desired index for analysis
filename = "D:\\CHANG\\Climate_models\\globalpatterns\\All\\" +TCP[T]+ "monthly.txt"
m = (-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.,-999.) #missing value array
f = (np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,) #filling value array
Index = np.genfromtxt(filename, names=True, missing_values=m, filling_values=f, usemask = True)
...
ax.plot(year,datarm, color = 'green', linestyle ='--')
ax.grid(True)
