...
for model in range(len(models)):
	...
	for rcp in range(len(rcps)):
		for i in range(n):
			patchdata[rcp].append(pt.get_patch(np.where(yprobs[model][rcp][i].data>0.407, 1,0)))
...
ax1 = plt.subplot(211)
plt.grid()
ax1.plot(t, ens_n45, color = 'blue', label = 'RCP 4.5 no. patches', lw= 2)
ax1.set_ylabel('Number of patches', color = 'blue')
for tl in ax1.get_yticklabels():
	tl.set_color('b')
ax1.set_xlabel('Year')
ax2 = ax1.twinx()
ax2.plot(t, ens_mu45*(.8**2), color = 'red', ls ='--', label = 'RCP 4.5 Median patch size', lw= 2)
...
plt.grid()
ax12.plot(t, ens_n85,  color = 'blue', label = 'RCP 8.5 no. patches', lw= 2)
ax12.set_ylabel('Number of patches', color = 'blue')
for tl in ax12.get_yticklabels():
	tl.set_color('b')
ax12.set_xlabel('Year')
ax22 = ax12.twinx()
ax22.plot(t, ens_mu85*(.8**2), color = 'red', ls ='--', label = 'RCP 8.5 Median patch size', lw= 2)
...
plt.savefig('ENS_AVG_patches.png', bbox_inches='tight')
plt.show()
#===================
#3D plot?
...
threshcells45 = []
#looking for the varicance terms
for i in range(n+1):
	yearprobs45 = []
