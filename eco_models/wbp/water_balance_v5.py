...
def Headerextract(gcm='n'):
	...
	if gcm =='n':
		...
		filename = "E:\\GCM\\gcm_averages\\TIFF\\rcp45\\tmin\\gcmavg_rcp45_tmin_2010_1.tif" 
...
def GCMextract(beginyear,endyear,var,rcp,mod):
	gcmlist = ['access1-0', 'cesm1-bgc','cesm1-cam5', 'cmcc-cm', 'cnrm-cm5', 'csiro-mk3-6-0', 'fgoals-g2', 'fio-esm'] #GCM list that are currently working
	model = gcmlist[mod]
	if (var == 'ppt'):
		v = 'pr'
	elif (var =='tmin'):
		...
		v = 'tasmax'
	workspace = "E:\\GCM\\" + model + "\\"
	GCMdata = []
	for cyear in range(beginyear,endyear+1):
		filename = workspace + "BCSD_0.008deg_"+ v + "_Amon_" + model + "_rcp" + str(rcp) + "_r1i1p1_" + str(cyear) + "01-" + str(cyear) + "12.nc"
...
def WBextract(BeginYear,EndYear,var):        
	Pdata = [] #List to store all PRISMData object
	workspace = "E:\\PRISM\\WB\\"+ var +"\\"    
	for searchyear in range(BeginYear, EndYear+1): #looping through years of interest
		for filenum in range(1,13): #does not consider the annual mean filenum (#14)
			if filenum <10:
				filename = workspace + var + "_" + str(searchyear) + "_0" + str(filenum) + ".tif"
			else:
...
def Topoextract(gcm='n'):
	...
	ds = None #close files
	if gcm == 'y':
		return(aspect[1:,1:], slope[1:,1:], elev[1:,1])
	else:
...
def PETcalc(Ta,month,hl, gcm):    #This is based on the Thornthwaite equation
...
def PET(Ta,HL, gcm='n'):
    p=[]
    for i in range(len(Ta)):
        p.append(PETcalc(Ta[i].data,Ta[i].month,HL, gcm))
    return(p)
...
def SoilWHC(): #defines the soil water holding capacity, currently using STATSGO dataset with depth at 150cm
	header = Headerextract()
	soilPath = "D:\\CHANG\\GIS_Data\\soil\\WHC\\800m\\awc150_800m_Shift2_Clip1.tif" 
...
def WaterBalanceModel(BeginYear, EndYear, burnin = 10, **kwargs):
	...
	soilmout = []; aetout = []; petout = []; packout = []; runoffout = []; mi = []; arid = []; 
	i = 0 + (burnin*12)
...
def ExportData(data,varname,workspace):
	...
	for i in range(len(data)):
		if (data[i].month <10):
			filename = varname + '_' + str(data[i].year) +'_0'+ str(data[i].month)+'.tif'
		else:
