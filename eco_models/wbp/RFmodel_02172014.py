...
# the sampled data should have 'response' in the first column, and lat/lon as the 2nd and 3rd columns
	if mf == None:
		mf = round(len(sampledata.dype.names)/2) #if no maximum feature size is specified, then use half of the predictors
	...
	clasf = RandomForestClassifier(n_estimators=2000, criterion = 'gini',compute_importances = True, max_features= mf, min_samples_leaf=20, oob_score=True, bootstrap = True)
...
# element 3: false positive rate, true positive rate, receiver operator curve
	...
	#feature importance
	xfname= np.array(sampledata.dtype.names[2:])
	...
	roc_auc = auc(fpr,tpr)
	return([sxfeature,sfeatureimp],[cmx,cmxperc],[fpr,tpr,roc_auc])
...
#flist2 = ['tmax9', 'pack6', 'pack11', 'vpd7', 'tmin6', 'aet5', 'pet6', 'tmin3']
