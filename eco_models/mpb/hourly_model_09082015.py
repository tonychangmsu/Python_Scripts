...
def hourly_transform(year, AOA, t_dmin_prev = None, npartitions = 24):
	...
	hours = np.zeros((nhours,dim[0],dim[1])) #hours are initialized as 0
	unit_hours = np.zeros((nhours,dim[0],dim[1])) #unit hours are initialized as 0
	...
	for i in range(julian_day_start, julian_day_end):
		...
		for t_hour in range(len(bound_time)): #twenty four hour day division
			index = day_hour * npartitions + t_hour ##also problem
			#print(index)
			...
			if ((i != julian_day_start) and (t_hour == 0)): #for the first new day
				...
				unit_hours[index] = np.round((current_julian_hour-np.floor(current_julian_hour))*24) #return the hourly time in 24 format
	...
	return(hourly_temp, hours, t_dmin_prev)
	
def write_data(outname, h_temp, hours):
	...
	temperature = root_grp.createVariable('temperature', 'f4', ('time', 'lat', 'lon',))
	...
	for i in range(len(outname)):
		times[i] = np.mean(hours[i])
		temperature[i,:,:] = (h_temp[i]*100).astype(int)
	days[:,:,:] = hours
	root_grp.close()	
...
if __name__ == "__main__":
	...
	h_temp, hours, t_dmin_prev = hourly_transform(year, AOA)
