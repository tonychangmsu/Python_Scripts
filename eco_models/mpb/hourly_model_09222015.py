...
def write_data(outname, h_temp, unit_hours, year):
	...
	temperature = root_grp.createVariable('temperature', 'int16', ('time', 'lat', 'lon',))
...
def tiff_write(data,Nx,Ny,cellsize,yul,xll, path, name):   
	fileformat = "GTiff"
	nbands = 1
...
if __name__ == "__main__":
	...
	for year in range(startyear, endyear):
		...
		t_dmin_prev_next = gdal.Open('%st_dmin_prev\\%s_%s.tif'%(writespace,'t_dmin_prev',year-1)).ReadAsArray()
		h_temp, hours, unit_hours, t_dmin_prev = hourly_transform(year, AOA, t_dmin_prev_next)
		t_dmin_prev_next = t_dmin_prev
		#need to save the very last t_dmin_prev for the next year...
		#for now just write it in as a tiff.
		...
		write_data(outname, h_temp.astype(int16), unit_hours, year) #have to reduce the amount of memory used 
		tiff_write(t_dmin_prev-np.floor(t_dmin_prev), nrow, ncol, csize, yul, xll, '%st_dmin_prev\\'%writespace, '%s_%s.tif'%('t_dmin_prev',year))
		tout=timeit.time.time()
		print('Time = %s' %(tout-tin))
	#okay now loop and use the last t_dmin_prev for the next year.
