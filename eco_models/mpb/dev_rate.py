...
def dev_per_day(x,pa):
	...
	ndays=ndays-1 # set new size to number of days
	#dev_day = np.zeros((7,ndays))
	dev_day = []
	# produces an 8 no. life stages (rows) X ndays (columns) of daily developmental indices
	'''
	dev_day[0,:ndays]=np.trapz((blogan(y-5,pa[0,:])/24).T) #egg
	dev_day[1,:ndays]=np.trapz((blogan(y-5,pa[1,:])/24).T) #L1
	dev_day[2,:ndays]=np.trapz((blogan(y-10,pa[2,:])/24).T) #L2
	dev_day[3,:ndays]=np.trapz((typeiii(y,pa[3,:])/24).T) #L3
	dev_day[4,:ndays]=np.trapz((linear_rate(y,pa[4,:])/24).T) #L4
	dev_day[5,:ndays]=np.trapz((linear_rate(y,pa[5,:])/24).T) #pupae
	dev_day[6,:ndays]=np.trapz((stnrat(y,pa[6,:])/24).T) #Pre-ovipositional adult
	dev_day[7,:ndays]=np.trapz((gallery_ln(y,pa[7,:])/24).T) # Ovipositional adult
	'''
	dev_day.append(np.trapz((blogan(y-5,pa[0,:])/24).T)) #egg
	dev_day.append(np.trapz((blogan(y-5,pa[1,:])/24).T)) #L1
	dev_day.append(np.trapz((blogan(y-10,pa[2,:])/24).T)) #L2
	dev_day.append(np.trapz((typeiii(y,pa[3,:])/24).T)) #L3
	dev_day.append(np.trapz((linear_rate(y,pa[4,:])/24).T)) #L4
	dev_day.append(np.trapz((linear_rate(y,pa[5,:])/24).T)) #pupae
	dev_day.append(np.trapz((stnrat(y,pa[6,:])/24).T)) #Pre-ovipositional adult
	dev_day.append(np.trapz((gallery_ln(y,pa[7,:])/24).T)) # Ovipositional adult
...
def typeiii(t,p):
	...
	x2=p[0]*(1-np.exp(-tau))
	x3=x1+x2-p[0]
	z=np.where(x3>0, x3, 0)
	q=np.where(t>p[4],t,0) #check for T less than base Temperature
	y=z*q
	return(y)
...
def G_curve(d_d,ndays):
	...
	for i in range(te):
		...
		g[count]=modulo(m_d_e[0][0][7],365) # m_d_e(8) is median egg; m_d_e(7) is adult emergence
		ret_line.append(i)
		count=count+1
...
def trap_devrats_new(dev_day,ndays,da_start,nyrs):
	...
	years_per_generation = np.zeros((nyrs,2))
	phase_space = np.zeros((nyrs,3))
	for i in range(0,nyrs):
		test=dev_day[0,da_start:ndays].cumsum(axis=0)
		if (len(test) == 0):
			continue
		else:
			if(np.max(test)<1.0):
				break
			for j in range(8):
				med_day_emerg[i,j]=(da_start+fnd1idx(test,1,1))
				test=dev_day[j,med_day_emerg[i,j]:ndays].cumsum(axis=0)
				if(np.max(test)<1.0):
					break
		...
		years_per_generation[i,0]=(med_day_emerg[i,7]-da_start)/365
		da_start=modulo(med_day_emerg[i,7],365) # modulo reduction
		phase_space[i,1]=modulo(med_day_emerg[i,7],365)
		if (da_start==0):
			da_start=1
		phase_space[i,1]=modulo(med_day_emerg[i,6],365)
...
def plot_g_function(g,return_line):
	...
	idx2 = len(np.where(g!=365)[0])
	plt.plot(np.arange(idx+1,idx2),g[idx+1:idx2])
	plt.plot([idx, idx+1],[g[idx], g[idx+1]],'r')
	plt.plot([1, idx2],[g[0], g[idx2]],'r')
	plt.show()
	return(idx)
