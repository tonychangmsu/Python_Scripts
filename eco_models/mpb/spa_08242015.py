...
def solar_transit_sunrise_sunset(unixtime, lat, lon, delta_t, is_grid = True):
	...
	cos_arg[np.abs(cos_arg) > 1] = np.nan
	...
	return(transit.astype(int), sunrise.astype(int), sunset.astype(int))
	
'''
def convert_unixtime_utc_local(unixtime, local_tz, after_epoch=True):
	...
	if after_epoch: #january 1 1970
		utctime = unixtime/60/60
		localtime = utctime + 25200
	return(utctime, localtime)
'''
	
#acquire all the lat, lon, and elevation data
#=======================MAIN======================#
delta_t = 67.0
pressure = 101325 / 100
temp = 12
...
#day = 1
#month = 1
#january 1, 1970 UTC = 0 to US/Mountain = 25200
...
print("Function took: %s seconds" % (tout-tin))
'''

#writing a netCDF4 file
#year = 2001
#our first goal should be to run this for the years 2000 - 2015, since those are the years for MODIS
#there is also a likely chance that we could run 2 sessions at the same time thus reducing processing time by 2 days
months = np.arange(1,13)
days = np.array([31,28,31,30,31,30,31,31,30,31,30,31])

for year in range(1985, 2016):
	writespace = 'K:\\NASA_data\\solar_out\\'
	writefile = '%s_snsrss.nc'%(year)
	root_grp = nc.Dataset('%s%s'%(writespace, writefile), 'w', format='NETCDF4')
	root_grp.description = 'Transit, sunrise, sunset times'
	...
	# variables
	times = root_grp.createVariable('time', 'i', ('time',))
	latitudes = root_grp.createVariable('latitude', 'f4', ('lat',))
	longitudes = root_grp.createVariable('longitude', 'f4', ('lon',))
	transit = root_grp.createVariable('transit', 'i', ('time', 'lat', 'lon',))
	sunrise = root_grp.createVariable('sunrise', 'i', ('time', 'lat', 'lon',))
	sunset = root_grp.createVariable('sunset', 'i', ('time', 'lat', 'lon',))

	# descriptions
	latitudes.units = 'degrees_north'
	longitudes.units = 'degrees_east'
	transit.units = 'Seconds since UNIX timestamp UTC (Epoch on January 1, 1970)'
	sunrise.units = 'Seconds since UNIX timestamp UTC (Epoch on January 1, 1970)'
	sunset.units = 'Seconds since UNIX timestamp UTC (Epoch on January 1, 1970)'
	...
	if is_leap_year(year):
		days[1] = 29 #if leap year add a day to February
	else:
		days[1] = 28 #change back if a leap year passed
		
	for month in months:
		for day in range(1,days[month-1]+1):
			print('%s/%s/%s'%(month, day, year))
			unixtime = np.ones(grid_dimension)*unix_time_convert(year, month, day, local_tz)
			sn, sr, ss = solar_transit_sunrise_sunset(unixtime, lat, lon, delta_t)
			transit[i,:,:] = sn
			sunrise[i,:,:] = sr
			sunset[i,:,:] = ss
			times[i] = i
			i += 1 #counter of the days
	# group
...
#let's think about a way to convert from UTC unixtime back to standard date/time
'''
def convert_from_unix(unixtime, uyear):
	day_array = np.array([31,28,31,30,31,30,31,31,30,31,30,31])
	...
	seconds_per_month = seconds_per_day*day_array
	year = 1970 + unixtime / seconds_per_year
	acc = unixtime - year * seconds_per_year
	for month in range(12):
		if seconds_per_month[month] < acc:
			acc -= month
			month += 1
...
unixtime_utc = unix_time_convert(year, month, day, local_tz)
"""
date = '%s/%s/%s'%(year, month, day)
clock_time = "18:21:00"
naive = datetime.datetime.strptime("%s %s"%(date,clock_time), "%Y/%m/%d %H:%M:%S")
local_dt = local.localize(naive) #assign the time zone
utc_dt = local_dt.astimezone(pytz.utc) 
#this is correct (6 hour difference)
unixtime_local = calendar.timegm(local_dt.timetuple())
unixtime_utc = calendar.timegm(utc_dt.timetuple()) 
...
zen, app_zen, sel, app_sel, azi, eqt = solar_position(unixtime_utcs, lats, lons, elevs, pressure, temp, delta_t, atmos_refract)
