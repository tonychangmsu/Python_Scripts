...
def write_dev_comp_data(nc_ds, dev_comp_date, julian_start, year, nyear, outname, appnd = False):
	...
	if appnd: 
		...
		root_grp.description = 'Days to development completion since start date up to %s years' %(nyear)
...
for year in range(startyear,endyear):
	...
	for stage in range(last_stage):
		for i in range(nyears):
			filename =  "G:\\MPB\\daily\\stage_%s\\mpb_phen_stage_%s_%s.nc"%(stage+1,stage+1,year+i)
		...
		for julian_start in range(ndays): 
			...
			if stage == 7: #write out the last stage development
				...
				if julian_start == 0:
					...
					appnd = True
				write_dev_comp_data(ds, store_inx[stage][julian_start], julian_start+1, year, nyears, outname, appnd = appnd)
	'''
