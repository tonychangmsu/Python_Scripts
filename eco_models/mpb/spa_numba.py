...
import time
#timein = time.clock()
#out = solar_position(unixtime_utc, lat, lon, elev, pressure, temp, delta_t, atmos_refract)
#timeout = time.clock()
#print('time is %s'%(timeout-timein))
#for a single point it takes 0.006513 sec
#with jit it takes 0.000193
'''
...
unix_utcs = np.arange(0,n)+unixtime_utc
unixtime_utcs =np.tensordot(unix_utcs, np.ones((ncols, nrows)), axes = 0)
#unixtime_utcs = np.arange(n)+unixtime_utc

timein = time.clock()
for i in range(n):
	zen, app_zen, sel, app_sel, azi, eqt = solar_position(unix_utcs[i], lats[i], lons[i], elevs[i], pressure, temp, delta_t, atmos_refract)
timeout = time.clock()
print('time is %s seconds' %(timeout-timein))
#okay so we have in some way applied numba to the calculations, but still somewhat slow?
#performed 100 calculations in 14.8 sec. But this is for a single time point. This fails to determine the 

#sunrise and sunset and transit calculations. 
#when performed on an array of times it is: 22.27 sec for 5 iterations.
#
...
def solar_transit_sunrise_sunset(unixtime, lat, lon, delta_t, is_grid = True):
	...
	#out = np.array([transit, sunrise, sunset])
...
timein = time.clock()
out = solar_transit_sunrise_sunset(unix_utcs, lats, lons, delta_t)
timeout = time.clock()
print('time is %s seconds' %(timeout-timein))
