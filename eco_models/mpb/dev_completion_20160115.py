...
#we have a problem because we go through each stage first, but then need to figure out each day
#after the day is figured out. We do this to reduce the time of accessing the annual stage development
#data. Rather then every year....
first_stage = 1
last_stage = 8
...
for stage in range(last_stage+1):
	...
	for julian_start in range(ndays): 
		...
		if stage == 8: #write out the last stage development
			...
			outname = 'G:\\MPB\\dev_comp\\stage_%s_dev_comp_%s.nc'%(stage+1,year)
			if julian_start == 0:
				appnd = False
			else:
				appnd = True
