...
def write_dev_comp_data(nc_ds, dev_comp_date, julian_start, year, outname, append = False):
	...
	if append: 
		...
		times[0] = julian_start
		development_date[0,:,:] = dev_comp_date
		root_grp.close()
...
ncols = 504
ndays = 3
dev_comp_dates = np.zeros((ndays,stages, nrows, ncols)) #storage array
year = 1980
...
nyears = 5 #maximum number of year for development

for stage in range(1,9):
	for i in range(nyears):
		filename =  "K:\\NASA_data\\mpb_phen_out\\daily\\stage_%s\\mpb_phen_stage_%s_%s.nc"%(stage,stage,year+i)
	...
	for julian_start in range(1,ndays+1): 
		...
		if stage == 1:
			...
			idx[idx==0] = len(dev)-1 #does not complete stage for the year and julian start...
		prev_stage_end = index_array(idx)
		dev_comp_dates[julian_start-1,stage-1] = idx

		if stage == 8: #write out the last stage development
			#write the output
			#outname = 'K:\\NASA_data\\mpb_phen_out\\dev_comp\\stage_%s_dev_comp_%s.nc'%(stage,year)
			outname = 'G:\\MPB\\dev_comp\\stage_%s_dev_comp_%s3.nc'%(stage,year)
			if julian_start == 1:
				append = False
			else:
				append = True
			write_dev_comp_data(ds, dev_comp_dates[julian_start-1,-1], julian_start, year, outname, append = append)
'''
