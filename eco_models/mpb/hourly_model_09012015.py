...
# julian_day = 189
# julian_hour = np.linspace(189, 190, 25)
...
# t_dmax = julian_day + minutes[round(sunrise+((sunset - sunrise)/2))] #around 1:30 pm is the maximum 
# t_dmin = julian_day + minutes[round(sunset+((sunset - sunrise_next)/2)-1440)]
# t_dmin_next = julian_day + 1 + minutes[round(sunset+((sunset - sunrise_next)/2)-1440)]

# for i in julian_hour:
...
# julian_day_start = 189
# julian_day_end = 191
# t_dmin_prev = julian_day_start #initialize the first minimum
# minutes = np.linspace(0,1, 1441)
# #each hour unit is 1/24 or 0.041666
# P = []
# hours = []
# for d in range(julian_day_start, julian_day_end):
	# last_sunset = 21*60 + 17
	# sunrise = 5*60 + 42
	# sunset = 21*60 + 17
	# sunrise_next = 5*60 + 42
	...
	# #julian_hour = np.linspace(d,d+1,25) #need to redefine this so the bounds are by the next julian day minimum temperature
	# julian_hour = np.arange(t_dmin_prev, t_dmin_next, (1/24))
...
julian_day_start = 1
julian_day_end = 5
ndays = julian_day_end-julian_day_start
t_dmin_prev = julian_day_start #initialize the first minimum
minutes = np.linspace(0,1, 1441)
#each hour unit is 1/24 or 0.041666
P = []
...
for d in range(julian_day_start, julian_day_end):
	...
	transit = 12*60 +10
	sunset = 21*60 + 17
	sunrise_next = 5*60 + 30
	
	#t_dmax = d + minutes[round(sunrise+((sunset - sunrise)/2))] #around 1:30 pm is the maximum 
	t_dmax = d + minutes[round((sunset - transit)/2 + transit)]
	#t_dmin = d + minutes[round(sunset+((sunset - sunrise_next)/2)-1440)]
	t_dmin = d + minutes[round(sunrise)]
	#t_dmin_next = d + 1 + minutes[round(sunset+((sunset - sunrise_next)/2)-1440)]
	t_dmin_next = d + 1 + minutes[round(sunrise_next)]
	
	#julian_hour = np.linspace(d,d+1,25) #need to redefine this so the bounds are by the next julian day minimum temperature
	julian_hour = np.arange(t_dmin_prev, t_dmin_next, (1/24))
	for i in range(len(julian_hour)):
		currentP = Pt(P_max, P_min, t_dmin, t_dmax, t_dmin_next, julian_hour[i]) 
		if ((d != julian_day_start) and (i == 0)): #if this is the first hour, we need to reference the last day
			P.append((currentP + lastP)/2) # take the average between two days
			hours.append(julian_hour[i])
		elif (i == len(julian_hour)-1): #last iteration
			lastP = currentP
		else: #other iterations just save the hour and temperature
			P.append(currentP)
			hours.append(julian_hour[i])
	t_dmin_prev = julian_hour[-1] #save for the next iteration of the loop
	j += 1
P = np.array(P)
hours = np.array(hours)
#hours = np.array(julian_hour)
...
# also using the new solar position information for sunrise, sunset, transit and next sunrise
# if we used the net radiation calculated every hour, then we could just use the intercept of 0 net radiation as the maximum. 
# but that is much too much work for the moment. Get the model up and running so we can just get some development rates!
...
year = 1948
filename = '%s%s\\%s_%s.nc' %(workspace, 'tmax', 'tmax', year)
P_max = nc.Dataset(filename).variables['tmax'][:,min_y_i:max_y_i,min_x_i:max_x_i]

filename = '%s%s\\%s_%s.nc' %(workspace, 'tmin', 'tmin', year)
P_min = nc.Dataset(filename).variables['tmin'][:,min_y_i:max_y_i,min_x_i:max_x_i]

#get the sun position arrays
filename = 'K:\\NASA_data\\solar_out\\%s_snsrss.nc'%(year)
sp_ds = nc.Dataset(filename)
sunrise = ds_unix_time_convert(sp_ds, 'sunrise', i+1, year) * 60
transit = ds_unix_time_convert(sp_ds, 'transit', i+1, year) * 60
sunset = ds_unix_time_convert(sp_ds, 'sunset', i+1, year) * 60
sunrise_next = ds_unix_time_convert(sp_ds, 'sunrise', i+2, year) * 60
...
for d in range(julian_day_start, julian_day_end):
	#initialize the min and max temperatures and the timing of them
	t_dmax = d + minutes[np.round((sunset - transit)/2 + transit).astype(int)]
	t_dmin = d + minutes[np.round(sunrise).astype(int)]
	t_dmin_next = d + 1 + minutes[np.round(sunrise_next).astype(int)]
	
	#julian_hour = np.linspace(d,d+1,25) #need to redefine this so the bounds are by the next julian day minimum temperature
	julian_hour = np.arange(t_dmin_prev, t_dmin_next, (1/24)) ##need to fix this part here @t.chang 09.01.2015
	for i in range(len(julian_hour)):
		currentP = Pt(P_max, P_min, t_dmin, t_dmax, t_dmin_next, julian_hour[i]) 
		if ((d != julian_day_start) and (i == 0)): #if this is the first hour, we need to reference the last day
			P.append((currentP + lastP)/2) # take the average between two days
...
lon = nc_ds.variables['lon'][min_x_i:max_x_i]
lon_grid,lat_grid = np.meshgrid(lon, lat)
julian_time = (np.array(nc_ds.variables['time'][:])) + 0.5 #add 0.5 to get the julian day since TOPOWX uses the julian hour at 12:00pm
...
declination = solar_declination(julian_time)
