...
aphTmin = aphTmin - np.random.random(365)*10 #add noise to the Tmin values to increase likelihood of death
######################################################
#assign C here
...
#first every time step we need to calculate the L and G values which are a function of tau and R
	...
	s_t = survive(P_t, aphTmin[t], mpb.alpha, mpb.beta)
	if t == 0:	
		s.append(s_t)
	else:
		new_s = np.min(np.array([s[t],s_t]))
		s.append(new_s)
...
ds = pd.date_range('8/1/1990', periods=len(s[1:]))
#data = pd.Series(np.vstack((s[1:],aphTmin)).T)
#ts = pd.DataFrame(data, index = ds)
ts = pd.Series(s[1:], index=ds))
ttemp = pd.Series(aphTmin, index=ds)

fig, ax = plt.subplots()
ax2 = ax.twinx()
ts.plot(ax=ax, style='r-')
ttemp.plot(ax=ax2, style='b-')
ax.set_ylabel('$P(survival)$')
ax2.set_ylabel('Temperature ($^o$C)')

#maybe use pandas to generate a dataframe so that I can plot the dates.
#survival function looks like it is behaving.
#cool it works,
