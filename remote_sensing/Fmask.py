...
def Fmask(im,cldpix,sdpix,snpix,cldprob,num_Lst):
...
# bsq ENVI readable '*Fmask' image
	zen,azi,ptm,Temp,t_templ,t_temph,Water,Snow,Cloud,Shadow,dim,ul,resolu,zc = plcloud(im, num_Lst)
	clr_pct = fcssm(im,zen,azi,ptm,Temp,t_templ,t_temph,Water,Snow,Cloud,Shadow,dim,ul,resolu,zc,cldpix,sdpix,snpix)
	print('Fmask finished for %s with %.2f%% clear pixels\n',im,clr_pct)
	return(clr_pct)
