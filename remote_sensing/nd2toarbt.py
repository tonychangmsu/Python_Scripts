...
from lndhdrread import *

Tab_ES_Dist = np.array([
		[1 ,0.98331],
		[2 ,0.98330],
		[3 ,0.98330],
		[4 ,0.98330],
...
def nd2toarbt(filename, Tab_ES_Dist = Tab_ES_Dist, ledap_processed = True):
	# convert DNs to TOA ref and BT
	# Revisions:
	# Translated from matlab to Python3 (Chang 03/08/2016)
	...
	print('Reading in header information & TIF images\n')
	...
	if (Lnum >= 4 and Lnum <= 7):
		...
		if ledap_processed:
			bandfiles = [('%s\\%s'%(filename, x)) for x in os.listdir(filename) if (('toa_band' in x) and ((len(x) == 35)))]
			TOAref = np.zeros((ijdim_ref[0],ijdim_ref[1],6))# Band1,2,3,4,5,&7
			for i in range(len(bandfiles)):
				if i == 5:
					im_th = gdal.Open(bandfiles[i]).ReadAsArray() - 273.15
				elif i == 6:
					TOAref[:,:,-1] = gdal.Open(bandfiles[i]).ReadAsArray() 
				else:
					TOAref[:,:,i] = gdal.Open(bandfiles[i]).ReadAsArray()
			#find where pixels are saturated
			bandfiles_raw = [('%s\\%s'%(filename, x)) for x in os.listdir(filename) if (('band' in x) and ((len(x) == 31)))]
			im_B1 = gdal.Open(bandfiles_raw[0]).ReadAsArray()
			im_B2 = gdal.Open(bandfiles_raw[1]).ReadAsArray()
			im_B3 = gdal.Open(bandfiles_raw[2]).ReadAsArray()
			B1Satu = (im_B1==255)
			B2Satu = (im_B2==255)
			B3Satu = (im_B3==255)
			return([im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu])
			####END HERE IF LEDAPS PROCESSED####
		else:
			bandfiles = [('%s\\%s'%(filename, x)) for x in os.listdir(filename) if (('band' in x) and ((len(x) == 31)))]
			for i in range(len(bandfiles)):
				if i == 5:
					im_th = gdal.Open(bandfiles[i]).ReadAsArray()
				else:
					#locals()['im_B{0}'.format(i+1)] = gdal.Open(bandfiles[i]).ReadAsArray()
					#equivalent to below
					
					im_B1 = gdal.Open(bandfiles[0]).ReadAsArray()
					im_B2 = gdal.Open(bandfiles[1]).ReadAsArray()
					im_B3 = gdal.Open(bandfiles[2]).ReadAsArray()
					im_B4 = gdal.Open(bandfiles[3]).ReadAsArray()
					im_B5 = gdal.Open(bandfiles[4]).ReadAsArray()
					im_B6 = gdal.Open(bandfiles[5]).ReadAsArray()
					im_B7 = gdal.Open(bandfiles[6]).ReadAsArray()
					
			#stopped here 02/23/2016 tchang
			
			# check to see whether need to resample thermal band
			if (reso_ref != reso_thm):
				# resmaple thermal band
				im_th = pixel2pixv([ul[1],ul[0]], [ul[1],ul[0]], resolu, [reso_thm,reso_thm], im_th, [ijdim_ref[1], ijdim_ref[0]], [ijdim_thm[1],ijdim_thm[0]])
			
			# Band7
			# n_B7=dir('*B7*')
			# im_B7=single(imread(n_B7.name))
			# only processing pixels where all bands have values (id_mssing)
			...
			for i in range(len(bandfiles)):
				if i == 5:
					im_th = ((Lmax[i] - Lmin[i]) / (Qcalmax[i] - Qcalmin[i])) * (im_th - Qcalmin[i]) + Lmin[i]
				else:
					locals()['im_B{0}'.format(i+1)] = ((Lmax[i]-Lmin[i])/(Qcalmax[i]-Qcalmin[i]))*(locals()['im_B{0}'.format(i+1)]-Qcalmin[i])+Lmin[i]
			...
			#		continue
			#	else:
			#		locals()['im_B{0}'.format(i+1)] = 10**4 * np.pi * locals()['im_B{0}'.format(i+1)] * dsun_doy**2/(ESUN[i]*np.cos(s_zen))
			im_B1 = 10**4 * np.pi * im_B1 * dsun_doy**2/(ESUN[0]*np.cos(s_zen))
			im_B2 = 10**4 * np.pi * im_B2 * dsun_doy**2/(ESUN[1]*np.cos(s_zen))
			im_B3 = 10**4 * np.pi * im_B3 * dsun_doy**2/(ESUN[2]*np.cos(s_zen))
			im_B4 = 10**4 * np.pi * im_B4 * dsun_doy**2/(ESUN[3]*np.cos(s_zen))
			im_B5 = 10**4 * np.pi * im_B5 * dsun_doy**2/(ESUN[4]*np.cos(s_zen))
			im_B7 = 10**4 * np.pi * im_B7 * dsun_doy**2/(ESUN[6]*np.cos(s_zen))
			...
			TOAref[:,:,5] = im_B7
			return([im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu])
	###did not finish this translation below for Landsat 8 from matlab to python portion###
	# '''
	# elif Lnum == 8:
		# # LPGS Upper lef corner alignment (see Landsat handbook for detail)
		# ul[0] = ul[0]-15
	...
	# '''
		...
		print('This sensor is not Landsat 4, 5, 7\n')
	return([im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu])
