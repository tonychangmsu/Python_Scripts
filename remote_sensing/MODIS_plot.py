...
def plot_refl(mod_array):
	nbands = len(mod_array)
	f, ax = plt.subplots(nrows = 1, ncols =nbands, figsize=(20, 16), dpi=300)
	for i in range(nbands):
		if nbands ==1:
			ds_img = ax.imshow(np.ma.masked_where(mod_array[i] <0, mod_array[i]), cmap ='viridis')
			cbar = f.colorbar(ds_img, ax = ax,fraction=0.046, pad=0.04, orientation = 'horizontal')
		#mask the negative reflectance values as clouds
		else:
			ds_img = ax[i].imshow(np.ma.masked_where(mod_array[i] <0, mod_array[i]), cmap ='viridis')
			cbar = f.colorbar(ds_img, ax = ax[i],fraction=0.046, pad=0.04, orientation = 'horizontal')
			ax[i].set_title('Band %s'%(i+1))
	plt.tight_layout()
	plt.show()

def plot_tassel_cap(b,g,w, savefile = None, date = None):
	f, ax = plt.subplots(nrows = 1, ncols =3, figsize=(16, 5), dpi=300)
	if savefile:
		f.suptitle("Normalized Tasseled Cap transforms for MOD09A1 Julian Day:%s"%(date), fontsize=16)
...
def mosaic_files(files_to_mosaic, first_band =1, last_band = 7, qc = True, reproj = False):
	'''
	...
	for band in range(len(mod_bands[0])):
		...
		if reproj:
			processed_mosaics.append(reproj_WGS84(mod_mosaic))
		else:
